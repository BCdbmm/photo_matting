...
def random_dilate(alpha, low=1, high=5, mode='constant'):
    """
    生成trimap的一种方式。utils里的方法get_trimap目的一样，这里做对比看效果
    :param alpha:
    :param low:
    :param high:
    :param mode:
...
def crop_offset(trimap, crop_size=2560):
    """
    生成用来随机裁剪的偏移量
    :param trimap:
    :param crop_size:
...
class MattingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, annotation_path, root_dir=None, transforms=None,
        ...
        if transforms is None:
            ...
            self._transforms = Transform.Compose([
                Transform.ColorJitter(brightness=32 / 255., contrast=0.5,
                                          saturation=0.5, hue=0.2),
                Transform.ToTensor(),
                Transform.Normalize(mean=channel_means, std=channel_std)])
    ...
    def __getitem__(self, index):
        ...
        if prob > 0.5:
            ...
            # trimap=trimap.transpose(PIL.Image.FLIP_LEFT_RIGHT)
        ...
        # trimap_=torch.Tensor(trimap/255.).unsqueeze(0)
        ...
        # image_concated=torch.cat([image_preprocessed,trimap_],dim=0)
        return image_concated,alpha_u,alpha_noise_u
        ...
        for image_path, _, alpha_path, _,trimap_path in image_matting_alpha_mask_paths:
            if root_dir is not None:
                if not image_path.startswith(root_dir):
                    ...
                    trimap_path=os.path.join(root_dir,trimap_path)
                    image_path = image_path.replace('\\', '/')
                    alpha_path = alpha_path.replace('\\', '/')
                    trimap_path=trimap_path.replace('\\','/')
                    print(trimap_path)
            image_alpha_paths.append([image_path, alpha_path,trimap_path])
        return image_alpha_paths

    def _remove_invalid_data(self):
        valid_data = []
        for image_path, alpha_path,trimap_path in self._image_alpha_paths:
            if not os.path.exists(image_path):
                continue
            if not os.path.exists(alpha_path):
                continue
            valid_data.append([image_path, alpha_path])
            # valid_data.append([image_path,alpha_path,trimap_path])
